...
public class ZeitintervallPersistierungsService {
     ...
     * - Für die über Fahrbeziehungspermutationen summierten und auch im Parameter übergebene Zeitintervalle
    ...
    public List<Zeitintervall> aufbereitenUndPersistieren(final List<Zeitintervall> zeitintervalle, final boolean kiAufbereitung) {
         ...
         *   alle möglichen Fahrbeziehungspermutationen summiert.
         */
        final List<Zeitintervall> summierteFahrbeziehungen =
                ZeitintervallFahrbeziehungsSummationUtil.getUeberFahrbeziehungSummierteZeitintervalle(zeitintervalle);
         ...
         *   werden die gleitenden Spitzenstunden ermittelt.
         */
        final List<Zeitintervall> gleitendeSpitzenstunden =
                ZeitintervallGleitendeSpitzenstundeUtil.getGleitendeSpitzenstunden(allPossibleFahrbeziehungen);

        /**
         * - Für die über Fahrbeziehungspermutationen summierten und auch im Parameter übergebene Zeitintervalle
         *   werden die Summen für die einzelnen {@link Zeitblock}e gebildet.
         */
        final List<Zeitintervall> summierteZeitbloecke =
                ZeitintervallZeitblockSummationUtil.getSummen(allPossibleFahrbeziehungen);

        /**
         * Für die im Parameter übergebenen Zeitintervalle werden die KI-Tagessummen ermittelt, wenn der boolean-Parameter true ist
         */
        ...
        if (kiAufbereitung) {
            ...
            try {
                final KIPredictionResult[] predictionResults = kiService.predictHochrechnungTageswerteForZeitIntervalleOfZaehlung(groupedZeitintervalleByFahrbeziehung);
                final List<Zeitintervall> zeitintervallForEachFahrbeziehung = ZeitintervallKIUtil.extractZeitintervallForEachFahrbeziehung(groupedZeitintervalleByFahrbeziehung);
                kiZeitintervalle.addAll(ZeitintervallKIUtil.createKIZeitintervalleFromKIPredictionResults(Arrays.asList(predictionResults), zeitintervallForEachFahrbeziehung));
    ...
    public List<Zeitintervall> checkZeitintervalleIfPlausible(final Zaehlung zaehlung, final int numberOfIntervalle) throws PlausibilityException {
        final List<Zeitintervall> zeitintervalle = zeitintervallRepository.findByZaehlungId(UUID.fromString(zaehlung.getId()), Sort.by(Sort.Direction.ASC, "startUhrzeit"));
        // überprüfen, ob alle Zeitintervalle vorhanden sind
        if (numberOfIntervalle == 0 || numberOfIntervalle == zeitintervalle.size()) {
            return aufbereitenUndPersistieren(zeitintervalle, List.of(Zaehldauer.DAUER_2_X_4_STUNDEN, Zaehldauer.DAUER_13_STUNDEN, Zaehldauer.DAUER_16_STUNDEN).contains(Zaehldauer.valueOf(zaehlung.getZaehldauer())));
        } else {
